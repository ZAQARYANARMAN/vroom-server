...
const register = async (req, res) => {
  try {
    ...
    sendMail(
      req.body.email,
      `http://localhost:3000/verifyEmail?onetimepass=${onetimepass}`
    );
...
const logIn = (req, res) => {
  try {
    createToken(res, { email: req.body.user.email, id: req.body.user._id });
...
const verifyEmail = async (req, res) => {
  try {
    ...
    const newUser = await User.findOneAndUpdate(
      { status: 0, onetimepass },
      { status: 1, onetimepass: null }
    );
    if (newUser) {
      createToken(res, { email: newUser.email, id: newUser._id });

      return res
        .status(201)
        .json({ message: "Congratulations, you have registered." });
    }
...
const getUser = async (req, res) => {
  try {
    let user = await User.findOne({ _id: req.body.token.id }).select(
      "username email fullname phoneNum"
    );
...
const authorInfo = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      return res
        .status(200)
        .json({ message: "User found", data: { user, cars } });
    } else {
